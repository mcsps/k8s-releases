...
* Input validations are being restored to the state file in this version of Terraform. Due to a state interoperability issue ([#33770](https://github.com/hashicorp/terraform/issues/33770)) in earlier versions, users that require interaction between different minor series should ensure they have upgraded to the following patches:
 README.md cert-manager ci cloud-provider-openstack conf.py docs external-dns index.rst kubernetes rancher requirements.txt setup.cfg setup.py terraform terraform-provider-opentelekomcloud tox.ini tpl Users of Terraform prior to 1.3.0 are unaffected;
 README.md cert-manager ci cloud-provider-openstack conf.py docs external-dns index.rst kubernetes rancher requirements.txt setup.cfg setup.py terraform terraform-provider-opentelekomcloud tox.ini tpl Terraform 1.3 series users should upgrade to 1.3.10;
 README.md cert-manager ci cloud-provider-openstack conf.py docs external-dns index.rst kubernetes rancher requirements.txt setup.cfg setup.py terraform terraform-provider-opentelekomcloud tox.ini tpl Terraform 1.4 series users should upgrade to 1.4.7;
 README.md cert-manager ci cloud-provider-openstack conf.py docs external-dns index.rst kubernetes rancher requirements.txt setup.cfg setup.py terraform terraform-provider-opentelekomcloud tox.ini tpl Terraform 1.5 series users should upgrade to 1.5.7;
 README.md cert-manager ci cloud-provider-openstack conf.py docs external-dns index.rst kubernetes rancher requirements.txt setup.cfg setup.py terraform terraform-provider-opentelekomcloud tox.ini tpl Users of Terraform 1.6.0 and later are unaffected.
...
* `terraform test`: Providers, modules, resources, and data sources can now be mocked during executions of `terraform test`. The following new blocks have been introduced within `.tftest.hcl` files:
 ...
 README.md cert-manager ci cloud-provider-openstack conf.py docs external-dns index.rst kubernetes rancher requirements.txt setup.cfg setup.py terraform terraform-provider-opentelekomcloud tox.ini tpl `mock_provider`: Can replace provider instances with mocked providers, allowing tests to execute in `command = apply` mode without requiring a configured cloud provider account and credentials. Terraform will create fake resources for mocked providers and maintain them in state for the lifecycle of the given test file.
 README.md cert-manager ci cloud-provider-openstack conf.py docs external-dns index.rst kubernetes rancher requirements.txt setup.cfg setup.py terraform terraform-provider-opentelekomcloud tox.ini tpl `override_resource`: Specific resources can be overridden so Terraform will create a fake resource with custom values instead of creating infrastructure for the overridden resource.
 README.md cert-manager ci cloud-provider-openstack conf.py docs external-dns index.rst kubernetes rancher requirements.txt setup.cfg setup.py terraform terraform-provider-opentelekomcloud tox.ini tpl `override_data`: Specific data sources can be overridden so data can be imported into tests without requiring real infrastructure to be created externally first.
 README.md cert-manager ci cloud-provider-openstack conf.py docs external-dns index.rst kubernetes rancher requirements.txt setup.cfg setup.py terraform terraform-provider-opentelekomcloud tox.ini tpl `override_module`: Specific modules can be overridden in their entirety to give greater control over the returned outputs without requiring in-depth knowledge of the module itself.
